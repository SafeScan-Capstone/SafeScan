        ...
        <motion.input
          ...
          animate={{
            borderColor: error
              ...
              : '#E5E7EA',
            boxShadow: error
              ? '0 0 0 3px rgba(238,68,63,0.12)'
              : focused
              ? '0 0 0 3px rgba(13,100,93,0.12)'
              : '0 0 0 0px transparent',
          }}
          transition={{ duration: 0.2 }}
          className={`w-full rounded-xl border-[1.5px] p-3 text-xs text-text-title placeholder-text-secondary outline-none bg-bg-input ${
            isPassword ? 'pr-11' : ''
          }`}
        />
      ...
      <AnimatePresence>
        {error && (
          <motion.p
            ...
            transition={{ duration: 0.2 }}
            className="text-xs text-danger mt-0.5 overflow-hidden"
          >
            {error}
          </motion.p>
        )}
